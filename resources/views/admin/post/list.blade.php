        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">View Post</h2>
            </header>
            <div class="panel-body">
              ...
              <div class="row">
                <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                  <a href="{{ route('admin.post.create') }}" class="brn btn-success btn-sm">New Post</a>
                </div>
              </div>
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                @foreach($posts as $val)
                  <tr class="@if(($loop->iteration % 2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="center hidden-phone p-2">
                      <a href="{{ route('admin.post.show', $val->id) }}" class="btn btn-sm btn-info"> <i
                          class="fa fa-eye"></i> </a>
                      <a href="{{ route('admin.post.edit', $val->id) }}" class="btn btn-sm btn-success"> <i
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('admin.ajax.update.post.status', '') }}"+'/'+id,
          method: "post",
          dataType: "html",
          data: {status: status},
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.post.destroy', '') }}"+'/'+pid,
          method: "delete",
          dataType: "html",
          success: function (data) {
            if (data === "success") {
